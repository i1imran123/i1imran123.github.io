...
<HEAD>
	...
	<div class="container">
    	<?php include 'menu.php'; ?>
         	<!-- AMFIRS - All NULL Data -->		<h5 class="red-text" style="font-family: Orbitron;"><b>Duplicate Beneficiaries (Same Voter Id - Same MFI)</b></h5>		<?php				//Formatting returned results			echo "<table class='bordered'>";		//table  border='0' padding='0'			echo "<tr class='deep-purple lighten-4 deep-purple-text text-darken-2'>";				echo "<th>";					echo "Sl No";				echo "</th>";				echo "<th>";					echo "ID";				echo "</th>";				echo "<th>";					echo "MFI Name";				echo "</th>";				echo "<th>";					echo "Beneficiary Name";				echo "</th>";				echo "<th>";					echo "District";				echo "</th>";				echo "<th>";					echo "Full Address";				echo "</th>";				echo "<th>";					echo "Voter Id";				echo "</th>";				echo "<th>";					echo "Total Outstanding Amount (Rs)";				echo "</th>";				echo "<th>";					echo "Count";				echo "</th>";			echo "</tr>";	$slno = 0;	$sumEligible = 0;	include 'dbconnectamfirs.php';	$sql = "SELECT id, name, mfiname, district, fulladdress, voterid,  SUM(outstanding) totaloutstanding, COUNT(*) cnt FROM mfirevised11 GROUP BY name, voterid, outstanding, mfiname HAVING count(*) > 1"; //Query for data extraction	$result = $conn->query($sql);	if ($result->num_rows > 0) {	    while($row = $result->fetch_assoc()) {				$id = $row['id'];				$mfi = $row['mfiname'];				$benName = $row['name'];				$address = $row['fulladdress'];				$district = $row['district'];				$voterId = $row['voterid'];				$totaloutstanding = $row['totaloutstanding'];				$count = $row['cnt'];				$slno = $slno + 1;				echo "<tr><td>".$slno."</td><td>".$id."</td><td>".$mfi."</td><td>".$benName."</td><td>".$district."</td><td>".$address."</td><td>".$voterId."</td><td>".$totaloutstanding."</td><td>".$count."</td></tr>";		}	   }		 else {		echo "0 results";	}?>  
